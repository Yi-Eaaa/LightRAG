...
async def extract_entities(
    ...
    entity_extract_prompt = PROMPTS["entity_extraction_without_description"]
    context_base = dict(
        tuple_delimiter=PROMPTS["DEFAULT_TUPLE_DELIMITER"],
        record_delimiter=PROMPTS["DEFAULT_RECORD_DELIMITER"],
        completion_delimiter=PROMPTS["DEFAULT_COMPLETION_DELIMITER"],
    ...
    async def _process_single_content(chunk_key_dp: tuple[str, TextChunkSchema]):
        ...
        for record in records:
            ...
            if if_entities is not None:
                if if_entities["entity_name"] in maybe_nodes:
                    duplicate_entity += 1
            ...
            if if_relation is not None:
                if (if_relation["src_id"], if_relation["tgt_id"]) in maybe_edges:
    ...
    save_exp_nodes = defaultdict(list)
    save_exp_edges = defaultdict(list)
    for idx, (m_nodes, m_edges) in enumerate(results):
        for k, v in m_nodes.items():
            save_exp_nodes[k].extend((item, f"chunk {idx}") for item in v)
        for k, v in m_edges.items():
            maybe_edges[tuple(sorted(k))].extend(v)
